...
class GetGoodView: UIView,UITextFieldDelegate,UITextViewDelegate {
    ...
    // MARK: - 设置数据
    @objc func setData() {
        self.tfNum.text = MemAddressModel.shared().shrphone
        self.tfArea.text = MemAddressModel.shared().area
        self.tfName.text = MemAddressModel.shared().shrname
        self.tfCity.text = MemAddressModel.shared().city
        self.tfProvince.text = MemAddressModel.shared().province
        self.tfView.text = MemAddressModel.shared().street
    }
...
extension GetGoodView {
    ...
    @objc func save(sender : UIButton) -> Void {
        ...
        GetGoodModel.shared.goodInfo(tfName: tfName, tfNum: tfNum, tfProvince: tfProvince, tfCity: tfCity, tfcityLocal: tfCity, tfArea: tfArea, tfDetailAddress: tfView) { (result) in
            CCog(message: result)
            if result {
                PersonInfoModel.shared.saveInfo(tfNickName: MemModel.shared().nickname, comfun: { (isSuccess) in
                    CCog(logError: isSuccess)
                })
