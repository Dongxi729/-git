...
class PersonInfoViewController: BaseViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if let headUrl = PersonInfoModel.shared.personImg {
            if (headUrl.contains(comStrURL)) {
            } else {
                let imgUrl = "http://" + comStrURL + "/" + headUrl
                
                if imgUrl.count > 0 {
                    self.headImg.downloadURL(down: imgUrl, finished: { (xxx) in
                        DispatchQueue.main.async {
                            self.headImg.image = UIImage.init(data: xxx)
                        }
                    })
                }
            }
...
extension PersonInfoViewController {
    func setUI() -> Void {
        ...
        //从本地加载服务器的头像地址
        if var headImgURL = dic["img"] as? String {
            if headImgURL.starts(with: "http://wx.qlogo.cn/mmopen") ||
                headImgURL.starts(with: "http://q.qlogo.cn/") {
                headImg.downloadURL(down: headImgURL, finished: { (xxx) in
                    DispatchQueue.main.async {
                        self.headImg.image = UIImage.init(data: xxx)
                    }
                })
            } else if (headImgURL.contains(comStrURL)) {
                headImg.downloadURL(down: headImgURL, finished: { (xxx) in
                    DispatchQueue.main.async {
                        self.headImg.image = UIImage.init(data: xxx)
                    }
                ...
                headImgURL = "http://" + comStrURL + "/" + PersonInfoModel.shared.personImg!
                headImg.downloadURL(down: headImgURL, finished: { (xxx) in
                    DispatchQueue.main.async {
                        self.headImg.image = UIImage.init(data: xxx)
                    }
...
extension PersonInfoViewController {
    ...
    @objc fileprivate func seleImgSEL() -> Void {
        ...
        UploadHeadTool.shared.choosePic { (data, resultData) in
            ...
            //上传好的头像地址
            if let headUrl = (resultData["data"] as? NSDictionary)?.object(forKey: "img") as? String {
                //传给单例
