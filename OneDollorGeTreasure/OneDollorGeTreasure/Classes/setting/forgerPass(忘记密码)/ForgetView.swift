...
extension ForgetView {
    ...
    @objc func saveSEL() -> Void {
        ...
        ForgetModel.shared.forgetPass(tfNum: tfNum, tfAuto: tfAuto, tfPass: tfPass, tfConfirmPass: tfComPass) {[weak self] (result) in
            guard let weakSelf = self else {return}
            if result {
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.5, execute: { 
                    weakSelf.delegate?.findPassSuccess()
                })
...
extension ForgetView {
    ...
    @objc func whenSendBtnClicked(button: CountDownBtn) {
        ...
        self.endEditing(true)
        SendAutoModel.shared.sendForgetAuto(sendType: 2, tfNum: tfNum) {[weak self] (result) in
            guard let weakSelf = self else {return}
            if result == "true" {
                button.initwith(color: #colorLiteral(red: 0.9434959292, green: 0.3252089322, blue: 0.3240017891, alpha: 1), title: "发送验证码", superView: weakSelf)
            }
        }
    }
}
